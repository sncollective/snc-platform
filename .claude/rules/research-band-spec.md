...
These per-control stores — the catalogs, and the dials' soft-enums — are exactly what [CATALOGS.md](research-band-catalogs.md) publishes at their v0.1 baseline. The *catalog* named here and the *baseline catalogs* in that document are one notion at two scopes: a single control's option-store, and the published collection of them all.
