...
- [x] **`--tab-bar-height` token** — 56px mobile / 0px desktop
- [x] **Content padding** — `.main-content` includes `--tab-bar-height`
- [x] **GlobalPlayer stacking** — bottom offsets + imperative padding removed
- [x] **Upload indicator stacking** — bottom offset
- [x] **Bottom tab bar** — `bottom-tab-bar.tsx` (Home, Feed, Live, Creators), z-150
- [x] **UserMenu visibility** — visible at all widths
- [x] **Root layout mount** — `<BottomTabBar />` in `__root.tsx`
- [x] **NavBar cleanup** — `<MobileMenu>` removed
- [x] **Delete mobile-menu chain** — 6 files deleted (component, CSS, hooks, tests)
- [x] **Responsive cleanup** — zero `max-width` media queries remain
...
- [x] `--tab-bar-height` is `56px` by default
- [x] `--tab-bar-height` is `0px` at `min-width: 768px`
...
- [x] `.main-content` padding-bottom sums all four variables
...
- [x] `.collapsedBar` bottom is `var(--tab-bar-height, 0px)` (shifts up on mobile)
- [x] `.collapsedOverlay` bottom includes `--tab-bar-height` in both base and desktop media query
- [x] Imperative `paddingBottom` on `document.body` removed from `global-player.tsx`
- [x] `--mini-player-height` CSS variable logic unchanged
...
- [x] Upload indicator sits above the tab bar on mobile
...
- [x] Tab bar renders 4 items: Home, Feed, Live, Creators
- [x] Active tab highlighted with accent color and `aria-current="page"`
- [x] Hidden on desktop (≥768px)
- [x] z-index 150 (below player, above nav)
- [x] Tab bar height matches `--tab-bar-height` (56px)
- [x] `aria-label="Primary navigation"` on nav element
...
- [x] `.loggedOut` is `display: flex` at all widths
- [x] `.avatarButton` is `display: flex` at all widths
- [x] `.avatarSkeleton` has no explicit `display` (block by default)
- [x] No responsive media query remains in `user-menu.module.css`
...
- [x] `BottomTabBar` rendered as peer to `NavBar` in `__root.tsx`
- [x] Renders on all pages (CSS handles desktop hide)
...
- [x] `MobileMenu` no longer imported or rendered in NavBar
- [x] `UserMenu` and `NotificationBell` remain in the `right` div
...
- [x] All 6 files deleted
- [x] No remaining imports of `mobile-menu`, `use-menu-toggle`, or `use-dismiss`
- [x] Build succeeds without these files
- [x] Stale comment in `auth-mock.ts` updated
...
- [x] No `@media (max-width: ...)` queries remain in any `.css` file under `apps/web/src/`
...
Signed off against the live dev environment — all 10 top-level tasks verified via code inspection (token in place, `.main-content` padding sums variables correctly, zero `@media (max-width: ...)` queries across web CSS, `<BottomTabBar />` mounted in `__root.tsx`, mobile-menu chain fully deleted). 15/15 layout-component unit tests passing.

**Architectural gap surfaced at epic level.** The tab bar ships 4 primary routes (Home, Feed, Live, Creators) as designed, but Studio, Merch, and Emissions now have no mobile entry point — the retired `<MobileMenu>` was their only path. The implementation is complete against this feature's brief; the gap is in the parent epic's scope. A new child story handles the fix:

- Sibling story: [`mobile-nav-overflow-access`](../stories/mobile-nav-overflow-access.md) — adds a 5th "More" tab that opens an overflow sheet with Studio / Merch / Emissions. Bound to 0.3.0 so it gates release. Until that story lands, the parent `mobile-nav-redesign` epic stays at `stage: review`.
