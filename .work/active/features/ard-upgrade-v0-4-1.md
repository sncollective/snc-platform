...
4. **Refactor `lint-research-claims.py`** — source category/status sets from local `ard-kernel/catalogs.json` (replace hardcoded `PATTERN_SPECS`); wire `ard-kernel/conformance/run.py` into the check path. *(code; needs grounding)*
   - **Finding surfaced during #3:** platform's *other* lint, `scripts/lint-citations.py`, is **261 diff-lines behind** the v0.4.1 reference (it hardcodes `PATTERN_CATEGORIES`; the v0.4.1 reference data-sources from `catalogs.json` with a built-in fallback, and adds a `--catalogs` arg). The cleanest move may be to **replace `scripts/lint-citations.py` wholesale with the vendored `scripts/ard-kernel/lint-citations.py`** (verbatim, data-sourced) rather than keep hand-maintaining it — which would **subsume the child-#2 SSRF hand-patch** (the reference already carries the v0.4.1 hardened `url_alive`). Decide in #4 whether #4 covers both lints or splits the `lint-citations.py` replacement into its own child.
