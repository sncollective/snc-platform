...
3. **Vendor `kernel/` in-tree** — copy `catalogs.json` + `discipline.md` + `conformance/` into platform; add `ARD-Version` stamps. *(mechanical copy-in + attribution)*
4. **Refactor `lint-research-claims.py`** — source category/status sets from local `catalogs.json` (replace hardcoded `PATTERN_SPECS`); wire `conformance/run.py` into the check path. *(code; needs grounding)*
