...
4. **Lint catch-up — split into two stories** (both reference the vendored `ard-kernel/`). The child-#3 finding (platform has *two* stale citation lints in `scripts/`) made one combined child the wrong shape:
   - **4a — Replace `scripts/lint-citations.py` with the vendored reference.** It's 261 diff-lines behind v0.4.1; the cleanest fix is to make it *be* `ard-kernel/lint-citations.py` (verbatim, data-sourced), which **subsumes the child-#2 SSRF patch**. Near-mechanical (swap + path resolution). → **pulled** as story `replace-lint-citations-with-vendored-reference` (stage: implementing).
   - **4b — Data-source `lint-research-claims.py` from `ard-kernel/catalogs.json`.** The original #4 target: replace hardcoded `PATTERN_SPECS` with a catalog loader (+ fallback), keep SNC matchers local, wire conformance. Genuine code refactor; needs grounding. → **pulled** as story `lint-research-claims-data-source-catalogs` (stage: implementing).
