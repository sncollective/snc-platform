...
Research in `.research/analysis/briefs/multi-platform-strategy.md` under the Auth Token Support section has prior exploration on this. The Better Auth JWT plugin (`better-auth/plugins/jwt`) is already installed and may provide a direct path — verify whether it satisfies the native app use case before designing a custom token mechanism.
