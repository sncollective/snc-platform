...
During the 0.3.0 refactor-gate, [fix-invite-email-frontend-url](../../.memory/archive/TODO-when-released) addressed one case where `config.BETTER_AUTH_URL` was used to build a user-facing frontend URL (the invite accept link) instead of the frontend hostname. The same anti-pattern exists at 5 other sites that were out of scope for the single-story fix:
