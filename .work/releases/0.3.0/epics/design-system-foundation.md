...
Foundation work for the UI/UX system. Token restructuring, icon system, first headless primitives (Ark UI), and shared pattern components. Reference: `.research/analysis/briefs/ui-ux-system-plan.md`.
...
Chosen over Radix (React-only, less CSS-Modules-natural), shadcn/ui (requires Tailwind), React Aria (heavier API). Lock-in risk is low — the visual layer is our CSS Modules, Ark UI is just the behavioral layer. Full comparison: `.research/analysis/briefs/ui-ux-system-plan.md § Recommended Architecture`.
...
`global.css` imports all token files, keeps resets and base utility classes (`.sr-only`, `.skip-link`, `.content-grid`). Zero migration — existing `var()` references keep working. Full token category reference: `.research/analysis/briefs/ui-ux-system-plan.md § Phased Roadmap → Phase 0`.
...
- **Full audit with severity ratings:** `.research/analysis/briefs/ui-ux-system-plan.md § Current State`
- **Library comparison tables:** `.research/analysis/briefs/ui-ux-system-plan.md § Recommended Architecture`
- **Token category details (what each file should contain):** `.research/analysis/briefs/ui-ux-system-plan.md § Phased Roadmap → Phase 0`
- **Responsive strategy (container queries, clamp, mobile-first):** `.research/analysis/briefs/ui-ux-system-plan.md § Phased Roadmap → Phase 3`
- **Accessibility approach and testing tools:** `.research/analysis/briefs/ui-ux-system-plan.md § Phased Roadmap → Phase 5`
- **Theme system for when designer joins:** `.research/analysis/briefs/ui-ux-system-plan.md § Phased Roadmap → Phase 6`
- **Existing UX research (content management patterns):** `.research/analysis/briefs/content-management-ux-patterns.md`
