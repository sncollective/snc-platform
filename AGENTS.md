...
- `bun run --filter @snc/e2e test` — Playwright golden-path e2e (staging env, localhost:3082). The PM2 dev API is started with explicit e2e-only profiles (`AUTH_RATE_LIMIT_PROFILE=e2e`, `TEST_CONTROL_PROFILE=e2e`) so the local command has the same deterministic setup surface as CI while production remains strict/fail-closed. First run on a fresh container: `bash scripts/dev/install-e2e-browsers.sh` (browsers install on demand, not in the devcontainer lifecycle)
...
Platform's persistent state splits across three top-level bands by output-class: `.memory/` (internal substrate), `.work/` (work items — output-class), and `.research/` (research output, operationalizing ARD). The conventions for each are in `.claude/rules/` (auto-loaded on the relevant paths) and summarized here.
...
### `.research/` — research band (ARD)

Platform adopts **ARD** (Agentic Research Discipline — MIT-licensed, agent-agnostic) directly from the ARD repo's kernel. The band carries external substrate + agent engagement with sources, read down-gradient only:
