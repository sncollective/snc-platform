...
.density-compact .v-highlight figure{width:56px}.density-compact .v-highlight{grid-template-columns:56px minmax(0,1fr)}
.density-tight{--s3:8px;--s4:10px;--s6:14px}
.density-tight .hero{height:168px}
.density-tight .bio,.density-tight .v-bio{line-height:14px}
.density-tight .deck{font-size:16px}
.density-tight .hero-copy h1{font-size:44px}
.density-tight .title h1{font-size:40px}
.density-tight .member figure,.density-tight .member img{width:56px;height:56px}
.density-tight .highlight figure,.density-tight .highlight img{width:56px;height:56px}
.density-tight .v-member figure{width:40px}.density-tight .v-member{grid-template-columns:40px minmax(0,1fr)}
.density-tight .v-highlight figure{width:48px}.density-tight .v-highlight{grid-template-columns:48px minmax(0,1fr)}
`;
 ...
 * Fits real-content volumes by walking a density ladder (normal → compact → tight); a fit
 * failure at every tier surfaces as BrowserPdfSinglePageFitError for the route to map. */
export const renderCreatorOneSheetPdf = async (input: {
  ...
  const orientation = input.orientation === "auto"
    ...
    : input.orientation;
  let lastFitError: BrowserPdfSinglePageFitError | null = null;
  for (const density of ["normal", "compact", "tight"] as const) {
    ...
    try {
      ...
      if (!(error instanceof BrowserPdfSinglePageFitError)) throw error;
      lastFitError = error;
    }
  }
  throw lastFitError;
