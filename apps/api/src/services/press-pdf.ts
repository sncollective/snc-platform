...
<section class="story"><span class="kicker">About</span>${content.shortBio ? `<p class="deck">${escapeHtml(content.shortBio)}</p>` : ""}<div class="bio"><p>${escapeHtml(horizontalBio[0] ?? "")}</p>${horizontalBio[1] ? `<p>${escapeHtml(horizontalBio[1])}</p>` : ""}</div>${content.pressQuotes.slice(0, 2).map(quoteBlock).join("")}${fans ? `<div class="fans grid"><span class="kicker">For fans of</span><p class="fan-line">${escapeHtml(fans)}</p></div>` : ""}</section>
