...
const SNC_INSTAGRAM_URL = "https://www.instagram.com/signaltonoise.co";
...
const renderMember = async (member: PressContent["members"][number], creatorId: string, vertical: boolean): Promise<string> => {
  ...
  const windowHeight = member.photo
    ...
    : null;
  const printWidth = vertical ? 188 : 225;
  const target = {
  ...
  const figure = windowHeight && member.photo
    ? photoFigure("", member.photo, src, `${member.name} portrait`, `width:${boxWidth}px;height:${windowHeight}px`)
...
const accentQuotedSegments = (text: string): string =>
  escapeHtml(text).replace(/\u201C(.+?)\u201D/g, '<span class="t-accent">$1</span>');
...
const oneSheetHtml = async (input: {
  ...
  const instagramLink = creator.socialLinks.find((link) => link.platform === "instagram");
  const instagramHandle = instagramLink?.url.replace(/\/+$/, "").split("/").pop();
...
<div class="hero">${leadFigure.replace(/^<figure class="hero">|<\/figure>$/g, "")}<div class="hero-copy"><h1>${escapeHtml(creator.displayName)}</h1><p class="facts">${content.location ? `<b>${escapeHtml(content.location)}</b>` : ""}${content.tagline ? accentQuotedSegments(content.tagline) : ""}</p></div></div>
<section class="story"><span class="kicker">About</span>${content.shortBio ? `<p class="deck">${escapeHtml(content.shortBio)}</p>` : ""}<div class="bio"><p>${escapeHtml(horizontalBio[0] ?? "")}</p>${horizontalBio[1] ? `<p>${escapeHtml(horizontalBio[1])}</p>` : ""}</div>${content.pressQuotes.slice(0, 2).map(quoteBlock).join("")}</section>
${fans ? `<div class="fans page-section grid"><span class="kicker">For fans of</span><p class="fan-line">${escapeHtml(fans)}</p></div>` : ""}
...
<footer class="actions grid"><div class="contact"><div class="c-line"><span>Press</span><strong>${escapeHtml(contact)}</strong></div>${booking ? `<div class="c-line"><span>Booking</span><strong>${escapeHtml(booking)}</strong></div>` : ""}<div class="c-line"><span>IG</span><a class="ig" href="${escapeHtml(SNC_INSTAGRAM_URL)}">@signaltonoise.co</a></div></div><div class="listen-group"><div class="listen"><span class="kicker">Listen · scan or type</span><a class="url" href="${escapeHtml(destinationUrl)}">${escapeHtml(humanUrl(destinationUrl))}</a>${destinations ? `<p class="destinations">${destinations}</p>` : ""}${instagramHandle && instagramLink ? `<p class="destinations ig-line"><a class="destination" href="${escapeHtml(instagramLink.url)}">@${escapeHtml(instagramHandle)}</a></p>` : ""}</div><div class="qr">${qr}</div></div></footer>${content.photographyCredits ? `<p class="photo-credits">Photography: ${escapeHtml(content.photographyCredits)}</p>` : ""}`;

  const verticalBody = `
<div class="spread">${leadFigure}<div class="copy"><header class="mast"><strong>S/NC · Press</strong><span>Artist one-sheet · ${new Date().getUTCFullYear()}</span></header><section class="title"><h1>${escapeHtml(creator.displayName)}</h1><p class="location">${content.location ? `<span>${escapeHtml(content.location)}</span>` : ""}${content.tagline ? `<span class="tagline">${accentQuotedSegments(content.tagline)}</span>` : ""}</p></section><section class="v-about"><span class="kicker">About</span>${content.shortBio ? `<p class="deck">${escapeHtml(content.shortBio)}</p>` : ""}<p class="v-bio">${escapeHtml(verticalBio)}</p></section>${content.pressQuotes[0] ? quoteBlock(content.pressQuotes[0]) : ""}${fans ? `<section class="v-fans"><span class="kicker">For fans of</span><p class="fan-line">${escapeHtml(fans)}</p></section>` : ""}${members.length ? `<section class="v-content"><span class="kicker section-label">Members</span><div class="v-members">${members.join("")}</div></section>` : ""}${highlights.length ? `<section class="v-content"><span class="kicker section-label">Highlights</span><div class="v-highlights">${highlights.join("")}</div></section>` : ""}${liveDatesLink ? `<section class="v-live"><span class="kicker">Live dates</span>${liveDatesLink}</section>` : ""}<footer class="v-actions"><div class="v-listen"><div><span class="kicker">Listen · scan or type</span><a class="url" href="${escapeHtml(destinationUrl)}">${escapeHtml(humanUrl(destinationUrl))}</a>${destinations ? `<p class="destinations">${destinations}</p>` : ""}${instagramHandle && instagramLink ? `<p class="destinations ig-line"><a class="destination" href="${escapeHtml(instagramLink.url)}">@${escapeHtml(instagramHandle)}</a></p>` : ""}</div><div class="qr">${qr}</div></div><div class="v-contact"><div class="contact"><span>Press</span><strong>${escapeHtml(contact)}</strong>${booking ? `<span>·</span><span>Booking</span><strong>${escapeHtml(booking)}</strong>` : ""}<span>·</span><span>IG</span><a class="ig" href="${escapeHtml(SNC_INSTAGRAM_URL)}">@signaltonoise.co</a></div></div></footer>${content.photographyCredits ? `<p class="photo-credits">Photography: ${escapeHtml(content.photographyCredits)}</p>` : ""}</div></div>`;
...
const releaseEpkSheet = async (
  ...
  const heroSpec = { slot: "banner" as const, width: 2294, height: 1163, gravity: "no" as const };
  ...
  const creditRows = [
    ...
    ...(release.writtenBy
      ? creditRow("Writers", release.writtenBy.split(/\s*·\s*/).map((writer) => `<span>${escapeHtml(writer)}</span>`).join(" · "), true)
      : []),
...
${story ? `<p class="story">${accentQuotedSegments(story)}</p>` : ""}
</div>
...
.hero-copy .facts b{color:var(--color-accent)}
...
.pull{border-left:2px solid var(--export-accent-decoration);padding:4px 0 4px 14px;font:400 20px/25px var(--font-display);margin:0 0 12px}
.story{margin:0;font-size:10.5px;line-height:17px;color:var(--color-text-muted)}.story .t-accent{color:var(--color-accent)}[data-theme="dark"] .story{color:var(--color-text)}
...
.row b{color:var(--color-text);font-weight:600;text-align:right;text-wrap:balance}.row b span{white-space:nowrap}
