...
"""SSRF-fence tests for lint-citations.py `url_alive` (ARD v0.4.1).

Zero-dependency stdlib unittest — no pytest, no network. The security-critical
guard (`_url_probe_safe` / `_host_is_public`) is deterministic offline for
literal-IP and bad-scheme hosts: `getaddrinfo` on a numeric host does not hit
DNS, so every case below resolves without a network call. The happy-path 200
response and live redirect-following are integration concerns (need a local
mock server) and are intentionally out of this offline unit set.
...
_SCRIPT = os.path.join(os.path.dirname(__file__), "..", "lint-citations.py")
_spec = importlib.util.spec_from_file_location("lint_citations", _SCRIPT)
lint = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(lint)


class UrlProbeSafeGuard(unittest.TestCase):
    """`_url_probe_safe` is the gate run before any connect — the SSRF fence."""

    def test_non_http_schemes_refused(self):
        for url in ("file:///etc/passwd", "gopher://h/x", "ftp://h/x", "data:text/plain,x"):
            self.assertFalse(lint._url_probe_safe(url), url)

    def test_cloud_metadata_endpoint_refused(self):
        # The canonical SSRF target — link-local.
        self.assertFalse(lint._url_probe_safe("http://169.254.169.254/latest/meta-data/"))

    def test_loopback_refused(self):
        self.assertFalse(lint._url_probe_safe("http://127.0.0.1/"))
        self.assertFalse(lint._url_probe_safe("http://[::1]/"))
    ...
    def test_unspecified_refused(self):
        self.assertFalse(lint._url_probe_safe("http://0.0.0.0/"))

    def test_missing_host_refused(self):
        self.assertFalse(lint._url_probe_safe("http://"))
    ...
    def test_public_literal_ip_allowed(self):
        ...
        for url in ("http://8.8.8.8/", "https://1.1.1.1/path"):
            self.assertTrue(lint._url_probe_safe(url), url)
